...
| :---: | :---: | :---: |
| $`\cdots`$ | \[11 4 11\] | $`\cdots`$ |
| $`\cdots`$ | \[11 3 6 11\] | $`\cdots`$ |
| $`\cdots`$ | \[11 2 4 7 12\] | $`\cdots`$ |
| $`\vdots`$ | $`\vdots`$ | $`\vdots`$ |
...
以上の観光周遊行動モデルとの比較用として，トリップチェイン全体の効用を考慮せず近視眼的に行動する旅行者を表現したNested Logitモデルのプログラムを，[nested-logitフォルダ](./nested-logit)に公開しています．
